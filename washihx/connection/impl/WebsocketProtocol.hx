...
class WebsocketProtocol extends Connection
{
	...
	override public function configuracion(_events:ServerEventManager, _server:IServer, _serializer:ISerializer = null):Void
	{
		super.configuracion(_events, _server, _serializer);
		_headers = [];
	}
	...
	override public function enConexion(cnxx:NetSock):Void
	{
		super.enConexion(cnxx);
	...
	function writeHeader(http:String):Void
	{
		...
		cnxx.writeBytes(Bytes.ofString(_headers.join("\r\n") + "\r\n\r\n"));
		_headers = [];
	}
	...
	*/
	override public function datosRecibidos(input:Input):Void
	{
		if (_useHttp)
		{
			var line:String;
		...
		else
		{
			...
			switch (recvFrame(input))
			{
				...
				case Binary(bytes): // binary
					recvBinary(bytes);
				case Close: // close
					conexionLost("close connection"); //Calls loseConnection which is implemented in Connection.hx
					//cnx.close();
				case Ping: // ping
					cnxx.writeBytes(createFrame(Pong)); // send pong
				case Pong: // pong
					// do nothing
			}
		}
	...
	function recvText(text:String):Void { super.recibir(text); }
	...
	*/
	...
	{
		cnxx.writeBytes(createFrame(Binary(bytes)));
	}
